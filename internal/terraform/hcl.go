...
type Generator struct {
	...
	privateLocationNames map[string]string
...
func NewGenerator(data *WorkspaceData) *Generator {
	g := &Generator{
		...
		privateLocationNames: make(map[string]string),
	...
	for _, l := range data.PrivateLocations {
		name := g.registry.Name("openstatus_private_location", l.GetName())
		g.privateLocationNames[l.GetId()] = name
	}
...
func (g *Generator) GenerateStatusPagesFile() *hclwrite.File {
	...
	for _, sp := range g.data.StatusPages {
		...
		writeCustomTheme(b, page.GetCustomTheme())
...
func (g *Generator) GeneratePrivateLocationsFile() *hclwrite.File {
	f := hclwrite.NewEmptyFile()
	body := f.Body()

	body.AppendUnstructuredTokens(hclwrite.Tokens{
		{Type: hclsyntax.TokenComment, Bytes: []byte("# Agent tokens are generated by OpenStatus and exposed as the read-only\n")},
		{Type: hclsyntax.TokenComment, Bytes: []byte("# `token` attribute. Reveal one with 'openstatus pl info <id> --show-token'.\n")},
	})
	body.AppendNewline()

	for _, l := range g.data.PrivateLocations {
		name := g.privateLocationNames[l.GetId()]
		b := body.AppendNewBlock("resource", []string{"openstatus_private_location", name}).Body()

		b.SetAttributeValue("name", cty.StringVal(l.GetName()))
		g.writeMonitorIds(b, l.GetMonitorIds())
		setStringMap(b, "metadata", l.GetMetadata())
...
func (g *Generator) GenerateImportsFile() *hclwrite.File {
	...
	for _, l := range g.data.PrivateLocations {
		writeImportBlock(body, "openstatus_private_location", g.privateLocationNames[l.GetId()], l.GetId())
	}
...
func (g *Generator) TotalResourceCount() int {
	...
	count += len(g.data.PrivateLocations)
	return count
}
...
func (g *Generator) HasPrivateLocations() bool {
	return len(g.data.PrivateLocations) > 0
}
...
func stringMapValue(m map[string]string) cty.Value {
	if len(m) == 0 {
		return cty.NilVal
	}
...
func setStringMap(b *hclwrite.Body, attr string, m map[string]string) {
	if v := stringMapValue(m); v != cty.NilVal {
		b.SetAttributeValue(attr, v)
	}
}

// writeCustomTheme emits only the modes that have entries: the provider rejects
// an empty map with "omit the map instead of leaving it empty".
func writeCustomTheme(b *hclwrite.Body, theme *status_pagev1.CustomTheme) {
	if theme == nil {
		return
	}
	modes := make(map[string]cty.Value, 2)
	if v := stringMapValue(theme.GetLight()); v != cty.NilVal {
		modes["light"] = v
	}
	if v := stringMapValue(theme.GetDark()); v != cty.NilVal {
		modes["dark"] = v
	}
	if len(modes) == 0 {
		return
	}
	b.SetAttributeValue("custom_theme", cty.ObjectVal(modes))
}
